...
def check_docker():
    ...
    if result[0] != 0:
        ...
        console.print(
            "This should have been installed from Docker's official repository",
            style="red",
        )
...
def get_instance_metadata(metadata_key):
    ...
    try:
        ...
        console.print(
            f"Error fetching instance metadata for {metadata_key}: {e}",
            style="bold red",
        )
        return None
...
def run_command(command, critical=False, timeout=300):
    ...
    try:
        ...
        if process.returncode != 0:
            ...
            console.print(
                f"Command failed with exit code {process.returncode}", style="bold red"
            )
            if critical:
...
def main():
    ...
    if docker_compose_url:
        console.print(
            f"Downloading Docker Compose file from: {docker_compose_url}",
            style="bold blue",
        )
    ...
    try:
        ...
        console.print(
            "This instance should have Docker pre-installed!", style="bold red"
        )
        sys.exit(1)
    ...
    if os.path.exists(docker_compose_path):
        ...
        console.print(
            f"docker-compose.yaml not found at {docker_compose_path}", style="bold red"
        )
        console.print("Checking for file in uploaded directories...", style="yellow")
        run_command(
            ["find", "/opt/uploaded_files", "-name", "*.yaml", "-o", "-name", "*.yml"]
        )
        run_command(
            ["find", "/tmp/uploaded_files", "-name", "*.yaml", "-o", "-name", "*.yml"]
        )
    ...
    run_command(
        ["sudo", "systemctl", "status", "sensor-log-generator", "--no-pager", "-l"]
    )
...
def setup_sensor_service():
    ...
    try:
        ...
        if service_src:
            ...
            console.print(
                "sensor-log-generator.service not found in any location",
                style="bold red",
            )
            return
        ...
        if config_src:
            ...
            run_command(
                ["sudo", "chmod", "644", "/opt/sensor/config/sensor-config.yaml"]
            )
        else:
            console.print(
                "sensor-config.yaml not found in any location", style="bold red"
            )
            return
        ...
        status_result = run_command(
            ["sudo", "systemctl", "is-active", "sensor-log-generator"]
        )
        if status_result[0] == 0:
            ...
            run_command(
                ["sudo", "journalctl", "-u", "sensor-log-generator", "-n", "50"]
            )
...
def generate_node_identity():
    ...
    try:
        ...
        result = subprocess.run(
            ["python3", "/opt/uploaded_files/scripts/generate_demo_identity.py"],
            capture_output=True,
            text=True,
        )
        ...
        if result.returncode == 0:
            console.print(
                "Demo node identity generated successfully", style="bold green"
            )
...
if __name__ == "__main__":
    try:
        ...
        console.print("\n" + "=" * 50, style="bold cyan")
        console.print(
            "Scheduling system reboot in 10 seconds to ensure all services start properly...",
            style="bold yellow",
        )
        console.print("=" * 50 + "\n", style="bold cyan")
        ...
        run_command(
            [
                ...
                "-r",
                "+1",
                "Startup complete, rebooting to ensure containers restart",
            ]
        )
