...
class ConfigAdapter:
    ...
    def convert_to_spat_config(
        ...
        if cluster_config is None:
            if self.cluster_config is None:
                raise ValueError(
                    "No cluster config loaded. Call load_cluster_config first."
                )
    ...
    def save_spat_config(
        self,
        config_file: str = "config.yaml",
        spat_config: Optional[Dict[str, Any]] = None,
    ) -> None:
        """Save SPAT configuration to config.yaml."""
        if spat_config is None:
            if self.spat_config is None:
                raise ValueError(
                    "No SPAT config available. Call convert_to_spat_config first."
                )
        ...
        with open(config_path, "w") as f:
            ...
            f.write(
                "# This configuration manages Bacalhau compute nodes across cloud regions\n\n"
            )
    ...
    def create_deployment_structure(
        self, setup_commands: Optional[str] = None, run_commands: Optional[str] = None
    ) -> None:
    ...
    def convert_full_migration(
        self, cluster_file: str = "cluster.yaml", config_file: str = "config.yaml"
    ) -> None:
        ...
        print("   • Deployment structure: deployment/")
        print("\nNext steps:")
        print(f"   1. Review and update SSH key paths in {config_file}")
        print("   2. Test deployment with: amauo create")
        print("   3. Original cluster.yaml backed up as cluster.yaml.backup")


def migrate_config(
    cluster_file: str = "cluster.yaml", config_file: str = "config.yaml"
) -> None:
