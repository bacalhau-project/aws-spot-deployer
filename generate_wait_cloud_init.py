...
"""Generate cloud-init that waits for file upload from deploy_spot.py"""

def generate_wait_cloud_init(config):
    """Generate minimal cloud-init that sets up and waits for deployment bundle."""
...
if __name__ == "__main__":
    ...
    cloud_init = generate_wait_cloud_init(config)
    
    size = len(cloud_init.encode('utf-8'))
    base64_size = size * 4 / 3
    
    print(f"Wait cloud-init size: {size:,} bytes ({size/1024:.1f} KB)")
    ...
    with open("cloud-init-wait.yaml", "w") as f:
        f.write(cloud_init)