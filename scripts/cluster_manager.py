...
class ClusterManager:
    ...
    def get_sky_cluster_name(self) -> Optional[str]:
        ...
        if success:
            for line in stdout.split('\n'):
                line = line.strip()
                if line and not line.startswith('NAME') and not line.startswith('Enabled'):
                    parts = line.split()
    ...
    def check_prerequisites(self) -> bool:
        ...
        if Path("~/.aws/credentials").expanduser().exists() or os.getenv("AWS_ACCESS_KEY_ID"):
    ...
    def _show_completion_banner(self):
        ...
        self.console.print("│ • Check status: ./cluster-deploy status        │", style="green")
        self.console.print("│ • List nodes: ./cluster-deploy list            │", style="green")
        self.console.print("│ • SSH to cluster: ./cluster-deploy ssh         │", style="green")
        self.console.print("│ • View logs: ./cluster-deploy logs             │", style="green")
        self.console.print("│ • Destroy cluster: ./cluster-deploy destroy    │", style="green")
        self.console.print("└─────────────────────────────────────────────────┘", style="green")
        self.console.print()
    ...
    def cleanup_docker(self) -> bool:
        ...
        if result.stdout.strip():
            ...
            subprocess.run(["docker", "stop", self.docker_container], capture_output=True)
...
def main():
    ...
    parser.add_argument("command", choices=[
        "create", "destroy", "status", "list", "ssh", "logs", "cleanup", "check"
    ])
    ...
    parser.add_argument("--log-file", default="cluster-deploy.log", help="Log file path")
