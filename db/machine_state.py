...
class MachineStateManager:
    ...
    def _init_db(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS idx_machines_instance_id ON machines(instance_id)"
                )
            ...
            async with self.get_connection() as conn:
                ...
                """,
                    (
                        ...
                        private_ip,
                        status,
                        launch_time or datetime.now().isoformat(),
                        spot_request_id,
                        vpc_id,
                        subnet_id,
                        security_group_id,
    ...
    async def update_machine(self, instance_id: str, **kwargs) -> bool:
        ...
        try:
            async with self.get_connection() as conn:
                await conn.execute(
                    f"UPDATE machines SET {set_clause} WHERE instance_id = ?", values
                )
                await conn.commit()
        ...
        try:
            async with self.get_connection() as conn:
                async with conn.execute(
                    "SELECT * FROM machines WHERE instance_id = ?", (instance_id,)
                ) as cursor:
    ...
    async def start_deployment_run(
        self, run_id: str, config_snapshot: str = ""
    ) -> bool:
